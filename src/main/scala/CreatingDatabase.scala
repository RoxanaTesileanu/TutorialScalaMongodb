...
- you have to allow the port 27017 used by the localhost for mongo ($ sudo ufw allow 27107)
- you have to start the mongod server in a terminal ($ sudo service mogod start). At the end of the session you should close the server ($sudo service mongod stop). 
